...
def saveONNX(model, filepath):
    ...
    #                down_ratio=4,
    #                head_conv=256).cuda()
    #构建关键点检测模型   参数需要根据加速的模型进行修改   主要是hps hm_hp
    model1 = DLASeg('dla{}'.format(34), {'hm': 1, 'wh': 2, 'hps': 34, 'reg': 2, 'hm_hp': 17, 'hp_offset': 2},
                    pretrained=False,
                    down_ratio=4,
                    head_conv=256).cuda()
    #model1=create_model(arch='dla_34',heads={'hm': 80, 'wh': 2, 'reg': 2},head_conv=256).cuda()
    ...
    for key, v in new_state_dict.items():
        print (key)
    ...
    model1.load_state_dict(new_state_dict,False)
    print(type(model1))
    print("succeed!!!!!!!!!!!!!!!!!!!!!")
    #model1.cuda()
    ...
    #torch.onnx.export(model1,dummy_input,filepath,input_names=input_names, output_names=output_names)
    print("onnx !!!!!!!!!!!!!!")
...
if __name__ == '__main__':
    saveONNX('multi_pose_dla_3x.pth', 'multi_pose_dla_3x.onnx')
    get_engine_trt7('multi_pose_dla_3x.onnx', 'multi_pose_dla_3x.trt')
    # import glob
